...
# to do add function to handle the sequence of ints for the key

class Hill(Resource):

    def get(self):
        parser = Parsely()
        parser = parser.parser_hill()
        args = parser.parse_args()
        if args.mode == 0:
            return self.encode(args.message, args.dimension)
        return self.decode(args.message, args.key)

    def generate_key(self, dimension, test_flag):
        counter = 0
        if test_flag == 1:
        ...
        while True:
            ...
            inv_key, signal = self.modinv(key)
            if signal:
                break
        return key, inv_key
    ...
    def pad_message(self, msg, key_row):
        msg_size = len(msg)
    ...
    def resize_array(self, msg, dimension):
        msg_array = numpy.array(msg)
        msg_len = msg_array.shape[0]
        msg_array.resize((int(msg_len / dimension), dimension))
        return msg_array

    def encode(self, msg, dimension, test_flag=0):
        lower_message = [string.ascii_lowercase.index(i) for i in msg]
        padded_message = self.pad_message(lower_message, dimension)
        key, inv_key = self.generate_key(dimension, test_flag)
        msg_array = self.resize_array(padded_message, dimension)
        return numpy.mod(numpy.dot(msg_array, key), 26), inv_key

    def decode(self, encoded_matrix, key):
        return jsonify({'decoded_message': numpy.mod(numpy.dot(encoded_matrix, key), 26),
